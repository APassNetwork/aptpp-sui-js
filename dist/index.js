...
class SuiDomain {
	...
	format_domain_data(cur) {
		...
		if (!cur || !cur.data || !cur.data.fields || !cur.data.fields.contents) return cur;
		...
		for (let x of cur.data.fields.contents) {
			...
			if (format) {
				...
				switch (dt.type) {
					case 'string':
						if (dt.value.startsWith('string::')) dt.value = dt.value.replace('string::', '');break;
	...
	async getObject(object_id) {
		...
		try {
			...
			if (!ret) return null;
			if (ret.data && ret.data.content) return ret.data.content.fields;
		} catch (e) {
			console.error(e);
			return null;
		}
	...
	async getDomainObj(domain_or_address, cb) {
		...
		if (cb) {
			await this.getDomainRecord(domain, function (ret) {
				if (ret.status == Okay && ret.record) cb(new SuiDomainObject(ret.record));else cb(null);
			});
		} else {
			let ret = await this.getDomainRecord(domain);
			if (ret.status == Okay && ret.record) return new SuiDomainObject(ret.record);
			return null;
		}
	}
